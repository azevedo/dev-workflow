...
- **Fix application** — apply all fixes, Critical + High + Med-conf-100 (Critical + High at displayed confidence plus Medium only when confidence == 100), Critical + High only, or one-by-one with Accept/Skip per finding; runs targeted tests after applying
