...
- Reference-file placement is decided by **shareability**, not by convenience. A reference with a **single consumer** lives skill-local at `skills/<name>/references/<file>.md` and is cited by **bare relative path**. A reference with **two or more consumers** stays at the **repo root** `references/`, shared by both `skills/` and `agents/` — a skill-local copy cannot serve `agents/convention-checker.md`, and would force `html-rendering.md` to be duplicated across five skills, which that file's own single-source rule forbids. One mechanism explains both halves: **a skill resolves bundled paths relative to its own `SKILL.md`** — which is exactly why a bare path works for a skill-local reference (it resolves inside `skills/ba-<name>/`, where the file is), and why the same bare path aimed at a repo-root reference would resolve inside `skills/ba-<name>/` and miss. Hence three citation forms, **each correct for its reader**: skill → skill-local is bare relative (`references/<file>.md`); skill → repo root is plugin-root-anchored `${CLAUDE_PLUGIN_ROOT}/references/<file>.md`; agent → repo root is bare — `agents/convention-checker.md` cites it that way, and you must **not** add the plugin-root prefix there. All three spellings are correct for their reader; do not normalise them to one.
  **Migration note — placement follows consumer count, and consumer count changes.** Before adding a second consumer to a skill-local reference, run `grep -rn "references/<basename>" skills/ agents/` to enumerate every citer. Promoting the file to the repo root is a file move **plus** a re-spelling of each of those citations into the form its own reader requires; without running that grep there is no way to find them.
