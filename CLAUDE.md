...
- The reviewer rubric — the severity ladder and the confidence anchor set — is owned by the `## Code-Anchor & Confidence Grammar` section of `skills/ba-review/SKILL.md`; `README.md`'s §4 sentence keeps only the floors, merge math, and legacy mapping. The mirror is **mechanically enforced, not hand-maintained**: the `rubric-mirror` check in `scripts/check-invariants.mjs` pins the legal-value literal `N ∈ {0, 25, 50, 75, 100}` (exact string — whitespace drift FAILs) across `skills/ba-review/SKILL.md`, `skills/ba-review-plan/SKILL.md`, and the 7 `agents/*-reviewer.md`, and asserts every reviewer agent's citation of that section title resolves to a real heading. Renaming the section without updating the agents is a CI failure, not a silent dangling citation.
