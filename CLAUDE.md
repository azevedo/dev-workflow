...
- Reference-file placement is decided by **shareability**. Single consumer → skill-local at `skills/<name>/references/<file>.md`, cited **bare relative**. Two or more → repo root `references/`, since a skill-local copy cannot serve `agents/convention-checker.md` and would force `html-rendering.md` to be duplicated across every skill that renders HTML, which that file's own single-source rule forbids. One mechanism explains both: **a skill resolves bundled paths relative to its own `SKILL.md`** — so a bare path finds a skill-local file and misses a repo-root one. Hence three citation forms: skill → skill-local, bare (`references/<file>.md`); skill → repo root, `${CLAUDE_PLUGIN_ROOT}/references/<file>.md`; agent → repo root, bare (`agents/convention-checker.md` — never add the prefix there). Each is correct for its reader; do not normalise them to one. The mechanism is proven only under `claude --plugin-dir`, where plugin-root and cwd coincide — **unverified for an installed plugin**, so read a first installed-plugin failure as this claim breaking, not the citing skill.
  **Migration note.** Placement follows consumer count, and that changes. Before adding a second consumer to a skill-local reference, `grep -rn "references/<basename>" skills/ agents/` to enumerate its citers — promotion is a move **plus** a re-spelling of each into its reader's form. **Hand-maintained, not CI-verified:** the `references` check walks top-level `references/` only, so skill-local files get no cited-at-least-once coverage, and its needle matches bare and anchored spellings alike, so an un-re-spelled promotion passes green and fails only at runtime.
