...
- `/ba-review-plan [path]` — Judged section-scoring plan review: a selection ledger over the 7 built-in reviewers with per-finding confidence and a soft gate (no external discovery); accepts a per-run `model:<value>` token
...
- `/ba-review [ref range]` — Post-implementation code review with built-in and discovered reviewers; accepts a per-run `model:<value>` token
...
- `comment-quality-reviewer` — Doc comments on declarations with callers, inline comments in changed bodies (built-in reviewer; on `/ba-review`'s roster only)
...
- Reference-file placement is decided by **shareability**. Single consumer → skill-local at `skills/<name>/references/<file>.md`, cited **bare relative**. Two or more → repo root `references/`. Three citation forms, each correct for its reader: skill → skill-local, bare; skill → repo root, `${CLAUDE_PLUGIN_ROOT}/references/<file>.md`; agent → repo root, bare; do not normalise them to one — two of the three would break. Before giving a skill-local reference a second consumer, `grep -rn` its citers and re-spell each; promotion is a move plus N citation edits, and CI pins none of this. Mechanism, evidentiary limits, and the promotion procedure: `.claude/agent_docs/prompt-authoring.md`.
...
- **The two reviewer counts differ and are not one fact.** `agents/` holds **eight** `*-reviewer.md` files; `/ba-review`'s roster is all **eight**; `/ba-review-plan`'s roster is **seven** — `comment-quality-reviewer` reviews code comments, a surface plan documents do not have, and stays reachable there via Adjust → Other. The counts are hand-maintained literals behind no single source of truth and pinned by no CI check, and in `README.md` and `CLAUDE.md` the only disambiguator is which skill's heading a sentence sits under. Eight-sites: `skills/ba-review/SKILL.md` (roster table, never-hide invariant, 2c, 2d, the bare numeral in the pre-judgment arithmetic, the Adjust ordering step), `README.md` `/ba-review` description + feature list. Seven-sites: `skills/ba-review-plan/SKILL.md` (roster intro and every count after it), `README.md` `/ba-review-plan` description + feature list, this file's `/ba-review-plan` skill line and never-hide bullet. Adding a ninth reviewer repeats the manual line-hunt; nothing verifies it was exhaustive.
...
- The reviewer rubric — the severity ladder and the confidence anchor set — is owned by the `## Code-Anchor & Confidence Grammar` section of `skills/ba-review/SKILL.md`; `skills/ba-review/SKILL.md` §4 (Step 4) owns the floors, merge math, and legacy mapping, and `README.md` carries only a user-facing summary of both. **CI pins two things and no more** — the legal-value literal and each reviewer agent's citation of the section title (see the `rubric-mirror` check in `scripts/check-invariants.mjs`, which owns the exact literal and the file list; don't restate them here). The literal is asserted **per `Task` dispatch block**, not per file: a dispatch template is the entire context of a fresh subagent, so a file-level test would pass a stripped template on the strength of a sibling's correct copy — which is exactly how two `general-purpose` templates once shipped reaching their subagent with no grammar at all. A mirror file with no `Task` block at all reads UNKNOWN, not PASS. So renaming the section without updating the agents is a CI failure rather than a silent dangling citation — though only for citations inside `agents/`: the ones in `README.md`, `skills/ba-review-plan/SKILL.md`, and this bullet are prose pointers CI does not pin, so walk them by hand on a rename. And **the ladder wording and the per-anchor meanings are still hand-mirrored across the `agents/*-reviewer.md` files and unchecked**. Change a severity's meaning and you must propagate it by hand; a green build does not mean the agents agree.
