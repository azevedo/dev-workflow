...
user: "Constraint: deepest-module. Brainstorm context: a new agent that audits a plan's acceptance criteria for testability — fires when a plan reaches review. Existing constraint set: agents/ (flat) uses role suffixes — -checker, -analyzer, -generator for workflow agents; -reviewer for review agents; -researcher, -locator, -finder for research agents. The agent dispatches read-only, surfaces violations only, never blocks. Dependencies: must read plan frontmatter; must compare findings across rounds."
